...
async def encrypt_async_byte_stream_to_file(
    ...
    If max_bytes is set, raises FileTooLargeError once the total plaintext read
    exceeds it (the caller is responsible for removing the partial output file).
    """
    ...
    async def write_encrypted_chunk(fout, plaintext_chunk: bytes) -> None:
        nonce = os.urandom(12)
        # Offload CPU-bound AES-GCM to a worker thread so a large upload never
        # blocks the event loop (health checks and other requests keep flowing).
        ciphertext = await asyncio.to_thread(
            aesgcm.encrypt, nonce, plaintext_chunk, None
        )

        await fout.write(struct.pack(">I", 12 + len(ciphertext)))
        await fout.write(nonce)
        await fout.write(ciphertext)

    async with aiofiles.open(output_filepath, "wb") as fout:
        await fout.write(struct.pack(">Q", 0))
        await fout.write(FILE_MAGIC_V2)
        await fout.write(struct.pack(">H", len(encrypted_key)))
        await fout.write(encrypted_key)
        ...
        async for incoming_chunk in byte_stream:
            ...
            while data_length - offset >= chunk_size:
                ...
                if max_bytes is not None and total_plaintext_size > max_bytes:
                    raise FileTooLargeError
                await write_encrypted_chunk(fout, plaintext_chunk)
        ...
        if pending:
            ...
            if max_bytes is not None and total_plaintext_size > max_bytes:
                raise FileTooLargeError
            await write_encrypted_chunk(fout, pending)

        await fout.seek(0)
        await fout.write(struct.pack(">Q", total_plaintext_size))
